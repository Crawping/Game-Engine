...
local function BuildAsset( i_assetInfo, i_extensions, i_builderFileName )
	...
	local path_source = s_AuthoredAssetDir .. i_assetInfo.name.."."..i_extensions.source
	local path_target = s_BuiltAssetDir .. i_assetInfo.name.."."..i_extensions.target
	...
	if shouldTargetBeBuilt then
		...
		do
			...
			if i_assetInfo.arguments then
				for i, argumentInTable in ipairs( i_assetInfo.arguments ) do
					arguments=arguments.." "..argumentInTable
				end
			end
...
local function BuildAssets( i_assetsToBuild )
	...
	for assetType, assetInfo in pairs( i_assetsToBuild ) do
		...
		local assetExtensions = assetInfo.extensions
		...
		for i, assetToBuild in ipairs( assetsToBuild ) do
			if not BuildAsset( assetToBuild, assetExtensions, builderFileName ) then
