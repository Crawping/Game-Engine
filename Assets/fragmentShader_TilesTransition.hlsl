	...
	out float4 o_color : COLOR0)
{
	...
	float range = i_color_perVertex.a % (0.2 + g_fade_constant);
	if (range<0.05 || range>0.15)
		...
		o_color = float4(color_sample * g_colorModifier * i_color_perVertex*(g_light_ambient + diffuseLighting), 1.0);
}
